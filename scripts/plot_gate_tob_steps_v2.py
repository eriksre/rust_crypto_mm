...
"""Plot Gate top-of-book (bid/ask) as step functions over time with trades.
...
try:
    ...
    raise SystemExit(
        ...
        "  .venv/bin/python scripts/plot_gate_tob_steps_v2.py ..."
    ) from e
...
def _load_gate_trades(path: Path, *, chunksize: int) -> pd.DataFrame:
...
def main() -> None:
    ...
    trades = _load_gate_trades(csv_path, chunksize=args.chunksize)
    ...
    plot_top_of_book(df, trades, title="Gate top-of-book (orderbook feed)")
