"""
Predict Gate mid-price using a (scalar) state-space Kalman filter.

Data source: logs/gate_activity.csv (event stream across venues).

Note: this script ignores all events from the Lighter exchange.
...
then run a causal filter over the full sample and plot predictions vs Gate orderbook mid.

Run:
  .venv/bin/python "scripts/lighter_pricing_model copy 3.py" --csv logs/gate_activity.csv --horizon-ms 500
"""
...
try:
    ...
    raise SystemExit(
        ...
        "  .venv/bin/python \"scripts/lighter_pricing_model copy 3.py\" ..."
    ) from e
...
def load_observations(
    ...
    *,
    ...
    if EXCLUDED_EXCHANGES:
        exch_s = df["exchange"].astype(str).str.lower()
        df = df[~exch_s.isin({str(x).lower() for x in EXCLUDED_EXCHANGES})].copy()
...
def fit_horizon_linear_model(
    ...
    *,
    ...
    div = div * disp_w
    m2 = np.isfinite(y) & np.isfinite(mom) & np.isfinite(div)
    y = y[m2]
    mom = mom[m2]
    div = div[m2]

    # Robustness: clip extreme values to avoid numerical issues during least-squares.
    mom = np.clip(mom, -0.02, 0.02)  # ±200 bps
    div = np.clip(div, -0.02, 0.02)
    y = np.clip(y, -0.05, 0.05)  # ±500 bps

    # OLS with intercept: y = b0 + b1*mom + b2*div
    X = np.column_stack([mom, div, np.ones(len(mom))])
...
def apply_horizon_linear_forecast(
    ...
    *,
    ...
    ok = np.isfinite(base) & np.isfinite(common) & np.isfinite(base_lag) & (base > 0) & (common > 0) & (base_lag > 0)
    lb = np.log(np.where(ok, base, np.nan))
    lc = np.log(np.where(ok, common, np.nan))
    ll = np.log(np.where(ok, base_lag, np.nan))
    mom = lb - ll
    div = lc - lb
    disp0 = max(float(common_disp0_bps), 1e-6)
    ...
    y_hat = beta[0] * mom + beta[1] * div + beta[2]
...
def print_eval_comparison(
    ...
    *,
    ...
    print(f"Evaluation @ {TARGET_EXCHANGE}:{TARGET_FEED} updates (predict current mid from prior info)")
    model_in_rmse, model_out_rmse, *_ = summarize("Kalman", "pred_mid_pre_target")
    b1_in_rmse, b1_out_rmse, *_ = summarize(f"Naive(prev {TARGET_EXCHANGE} mid)", "naive_prev_lighter")
    b2_in_rmse, b2_out_rmse, *_ = summarize("Naive(last non-target book mid)", "naive_last_ref_book")
    print(
        f"RMSE improvement vs naive(prev {TARGET_EXCHANGE}): "
        f"in {(b1_in_rmse - model_in_rmse):.2f}, out {(b1_out_rmse - model_out_rmse):.2f}"
    )
    print(
        "RMSE improvement vs naive(last non-target): "
    ...
    if eval_move_bps and eval_move_bps > 0:
        ...
        if kept >= 10:
            ...
            n1_in, n1_out = summarize_subset(f"Naive(prev {TARGET_EXCHANGE})", "naive_prev_lighter")
            n2_in, n2_out = summarize_subset("Naive(last non-target)", "naive_last_ref_book")
            print(
    ...
    if horizon_ms and horizon_ms > 0 and "pred_mid_pre_target_h" in eval_rows.columns:
        ...
        print(f"Evaluation @ {TARGET_EXCHANGE}:{TARGET_FEED} updates (predict mid at +{horizon_ms:g}ms, best-effort)")
        model_h_in, model_h_out = summarize_h("Kalman", "pred_mid_pre_target_h")
        b1_h_in, b1_h_out = summarize_h(f"Naive(prev {TARGET_EXCHANGE} mid)", "naive_prev_lighter")
        b2_h_in, b2_h_out = summarize_h("Naive(last non-target book mid)", "naive_last_ref_book")
        print(
            f"Horizon RMSE improvement vs naive(prev {TARGET_EXCHANGE}): "
            f"in {(b1_h_in - model_h_in):.2f}, out {(b1_h_out - model_h_out):.2f}"
        )
        print(
            "Horizon RMSE improvement vs naive(last non-target): "
            f"in {(b2_h_in - model_h_in):.2f}, out {(b2_h_out - model_h_out):.2f}"
        )

    # Streaming horizon eval: from any time t, predict target mid at t+h using only info <= t.
...
def plot_results(
    ...
    *,
    ...
    if has_h and not np.allclose(p["pred_mid_h"], p["pred_mid"]):
        ax0.step(
            p["t_dt"],
            p["pred_mid_h"],
            where="post",
            label=f"Predicted {TARGET_EXCHANGE.capitalize()} mid (+{horizon_ms:g}ms)",
            linewidth=1.4,
            color="red",
        )
    ...
    # Causal-at-target evaluation line: the prediction recorded just before consuming the target update.
    if "pred_mid_pre_target" in p.columns and "is_target" in p.columns:
        pre = p["pred_mid_pre_target"].to_numpy(dtype=np.float64)
        pre_mask = p["is_target"].to_numpy(dtype=bool) & np.isfinite(pre) & (pre > 0)
        if bool(pre_mask.any()):
            ax0.step(
                p.loc[pre_mask, "t_dt"],
                p.loc[pre_mask, "pred_mid_pre_target"],
                label=f"Pre-{TARGET_EXCHANGE.capitalize()} (before tick)",
                where="post",
                linewidth=1.0,
                alpha=0.9,
                color="darkorange",
    ...
    ax0.step(
        ...
        p["lighter_mid_ffill"],
        where="post",
        label=f"{TARGET_EXCHANGE.capitalize()} mid (ffill)",
        linewidth=1.0,
        alpha=0.8,
        color="blue",
    )

    # If horizon is enabled, overlay the realized target mid at t+h on the same time axis (t).
    if has_h:
        horizon_ns = int(horizon_ms * 1e6)
        lighter_updates = preds[preds["lighter_mid"].notna()][["t_ns", "lighter_mid"]].copy()
        if len(lighter_updates) > 2:
            ...
            ax0.step(
                ...
                realized_h,
                where="post",
                label=f"Realized {TARGET_EXCHANGE.capitalize()} mid (+{horizon_ms:g}ms)",
                linewidth=1.2,
                alpha=0.85,
            )
    ...
    ax1.set_title(
        f"Errors @ {TARGET_EXCHANGE}:{TARGET_FEED} updates{label_suffix}: "
        f"in RMSE={in_rmse:.2f} MAE={in_mae:.2f} p95={in_p95:.2f} dir={in_dir:.3f} | "
        f"out RMSE={out_rmse:.2f} MAE={out_mae:.2f} p95={out_p95:.2f} dir={out_dir:.3f}"
    )
...
def main() -> int:
    ...
    ap.add_argument("--out", type=str, default="plots/gate_mid_kalman.png")
    ...
    ap.add_argument("--lighter-r-mult", type=float, default=0.05, help="Scale measurement variance for the target orderbook (smaller => tracks target tighter).")
    ...
    ap.add_argument("--q-floor-mult", type=float, default=20.0, help="Floor q_per_sec using target realized variance * this.")
    ...
    ap.add_argument("--base-source", type=str, default="lighter_ffill", choices=["nowcast", "lighter_ffill"], help="Base 'current target price' used by the horizon model.")
    ...
    ap.add_argument("--snap-age0-ms", type=float, default=20.0, help="Nowcast snap: start snapping when target age exceeds this (ms).")
    ap.add_argument("--snap-age-scale-ms", type=float, default=10.0, help="Nowcast snap: sigmoid scale for target age (ms).")
    ap.add_argument("--snap-min-n", type=int, default=3, help="Nowcast snap: require at least this many non-target venues in the median.")
    ...
    if tuning.horizon_ms and tuning.horizon_ms > 0:
        if args.horizon_model == "ecm":
            ...
            print(f"Horizon linear model: beta_mom={beta[0]:.3f} beta_div={beta[1]:.3f} intercept={beta[2]:.3g}  train_rmse_log={rmse_log:.4g}")
        else:
    ...
    plot_results(
        ...
        split_ns=split_ns,
        out_path=args.out,
        title=f"{TARGET_EXCHANGE.capitalize()} mid prediction (Kalman state-space, +{tuning.horizon_ms:g}ms)",
        horizon_ms=tuning.horizon_ms,
