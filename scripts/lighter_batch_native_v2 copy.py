...
def main():
    ...
    # Submit each order via sendTx (like the SDK does)
    print(f"\n=== Submitting {len(signed_txs)} Orders ===")
    
    successful = []
    for i, (tx_type, tx_info) in enumerate(signed_txs):
        print(f"\nSubmitting order {i+1}/{len(signed_txs)}...")
        print(f"POST {BASE_URL}/api/v1/sendTx")
        ...
        if resp.status_code == 200:
            ...
            if data.get("code") == 200:
                print(f"✓ Order {i+1} submitted!")
                successful.append(data)
            else:
                print(f"✗ Order {i+1} failed: {data.get('message', 'unknown')}")
        else:
            print(f"✗ HTTP error")
    
    print(f"\n=== Summary ===")
    print(f"Submitted: {len(successful)}/{len(signed_txs)} orders")
