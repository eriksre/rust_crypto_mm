...
def main(data_path: Optional[Path] = None):
    ...
    # Add future lighter prices (accounting for cancellation latency)
    print(f"\nAdding future lighter prices with {LATENCY_OFFSET_NS/1e6:.0f}ms latency offset...")
    priors_df = add_future_lighter_prices(priors_df, df, LATENCY_OFFSET_NS)
    
    # Evaluate
    print("\n" + "=" * 60)
    print(f"Evaluation (predicting lighter price {LATENCY_OFFSET_NS/1e6:.0f}ms ahead)")
    print("=" * 60)
