...
"""Plot OKX top-of-book (bid/ask) as step functions over time with trades.
...
try:
    ...
    raise SystemExit(
        ...
        "  .venv/bin/python scripts/plot_okx_tob_steps.py ..."
    ) from e
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Step-plot OKX top-of-book bid/ask over time",
    )
    parser.add_argument(
        "csv",
        nargs="?",
    ...
    parser.add_argument(
        "--limit",
        type=int,
        help="Keep only the last N OKX orderbook rows (after filtering)",
    )
    parser.add_argument(
        "--out",
        type=str,
        help="Optional output image path (e.g. plots/okx_tob.png)",
    )
    parser.add_argument(
        "--no-show",
        action="store_true",
...
def _load_okx_orderbook(path: Path, *, chunksize: int) -> pd.DataFrame:
...
def _load_okx_trades(path: Path, *, chunksize: int) -> pd.DataFrame:
...
def plot_top_of_book(
    ...
    *,
    ...
    if df.empty:
        raise SystemExit("No OKX orderbook market rows found; nothing to plot.")
...
def main() -> None:
    ...
    df = _load_okx_orderbook(csv_path, chunksize=args.chunksize)
    trades = _load_okx_trades(csv_path, chunksize=args.chunksize)
    ...
    plot_top_of_book(df, trades, title="OKX top-of-book (orderbook feed)")
