...
def print_eval_comparison(
    ...
    *,
    ...
    def summarize(name: str, pred_col: str) -> Tuple[float, float, float, float, float, float]:
        ...
        print(
            f"{name}: "
            f"in RMSE={in_rmse:.2f} ({in_rmse_bps:.2f}bps) MAE={in_mae:.2f} ({in_mae_bps:.2f}bps) p95={in_p95:.2f} dir={in_dir:.3f} | "
            f"out RMSE={out_rmse:.2f} ({out_rmse_bps:.2f}bps) MAE={out_mae:.2f} ({out_mae_bps:.2f}bps) p95={out_p95:.2f} dir={out_dir:.3f}"
        )
    ...
    if horizon_ms and horizon_ms > 0 and "pred_mid_pre_target_h" in eval_rows.columns:
        ...
        def summarize_h(name: str, pred_col: str) -> Tuple[float, float]:
            ...
            print(f"{name} (horizon {horizon_ms:g}ms): in RMSE={in_rmse:.2f} ({in_rmse_bps:.2f}bps) | out RMSE={out_rmse:.2f} ({out_rmse_bps:.2f}bps)")
